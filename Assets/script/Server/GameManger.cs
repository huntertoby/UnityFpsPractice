...
public class GameManager : NetworkBehaviour
{
    ...
    private void RpcInitializePlayersForNewRound()
    {
        ...
        CmdGiveMoney(player, 2000);
        var ui = player.GetComponent<Ui>();
        ui.SetGameUiActive(true);
        fpsController.ChangeWeapon(0);
        fpsController.ResetWeaponAmmo();
        CmdSetMaxHealth(player);
        health.CmdIsDied(false);
        SetPlayerSide(netWorkPlayerControl);
        AddServerPerson(netWorkPlayerControl.team,player);
    }
    
    [ServerRpc(RequireOwnership = false)]
    public void CmdGiveMoney(Transform player,int money)
    {
        if(player.GetComponent<Bomb>())return;
        player.GetComponent<NetWorkPlayerControl>().money += money;
        RpcGiveMoney(player, money);
        Debug.Log("給"+player.GetComponent<NetWorkPlayerControl>().playerName+" "+money);
    }
    
    [Server]
    private void ServerGiveMoney(Transform player,int money)
    {
        if(player.GetComponent<Bomb>())return;
        player.GetComponent<NetWorkPlayerControl>().money += money;
        RpcGiveMoney(player, money);
    ...
    [ObserversRpc]
    private void RpcGiveMoney(Transform player,int money)
    {
        if (player.GetComponent<NetworkObject>().IsOwner)
        {
            player.GetComponent<NetWorkPlayerControl>().money += money;
            Debug.Log(player.GetComponent<NetWorkPlayerControl>().playerName+" 獲得 "+money+" $");
        }
    }
    ...
    private void CmdSetSpawnPlace()
    {
        ...
        Debug.Log(player.GetComponent<NetWorkPlayerControl>().playerName+" 在 "+team+" 隊 ");
    ...
    [ServerRpc(RequireOwnership = false)]
    public void CmdDealDamage(Transform whoDid,Transform toWho, float damage)
    {
        Debug.Log(damage);
        toWho.GetComponent<Health>().GotDamage(whoDid,damage);
    }
    
    [Server]
    public void ServerDealDamage(Transform whoDid,Transform toWho, float damage)
    {
        toWho.GetComponent<Health>().GotDamage(whoDid,damage);
    }
    ...
    {
        _bomb = bombTransform.gameObject;
    }
    ...
    [Server]
    public void ServerPlayerDie(int team,Transform whoDie,Transform killer)
    {
        Debug.Log("Team: "+team+" playerDIE");
        RpcPlayerDie(whoDie,killer);
        if (team == 1) _team1Person--;
        if (team == 2) _team2Person--;
        CheckWin();
        ServerGiveMoney(killer,1000);
    }
    
    [ObserversRpc]
    public void RpcPlayerDie(Transform whoDie,Transform killer)
    {
        whoDie.position = GameObject.Find("DieTransform").transform.position;
        var player = GameObject.Find("MyPlayer");
        player.GetComponentInChildren<KillEventUiControl>().CreatKillEventPrefab(killer,whoDie);
    }
    ...
    private void CheckWin()
    {
        ...
        if (_team1Person != 0 && _team2Person != 0) return;
        if(_timeOut) return;
    ...
    {
        GameObject.Find("MyPlayer").GetComponent<BuyWeaponMenu>().CloseBuyMenu();
    }
